...
class MakeHitAreaFromSequence(object):
	...
	def move_assets_to_content_bundle(self, incomingAssets):
		shutil.unpack_archive('__OverriddenContent__.zip', os.path.join(pathlib.Path.cwd()))
		shutil.rmtree(os.path.join(pathlib.Path.cwd()) + '/__MACOSX') 
		assetNumber = 1

		def updateCat(seqUUID, motiUUID, fullLang, ISOLang):
			#img seq
			shutil.rmtree(os.path.join(pathlib.Path.cwd()) + "/__OverriddenContent__/VideoAppsClipsAssets/Assets/" + seqUUID + "/AssetData/" + fullLang) 
			shutil.copytree(os.path.join(pathlib.Path.cwd()) + '/' + i + '/Media/', os.path.join(pathlib.Path.cwd()) + "/__OverriddenContent__/VideoAppsClipsAssets/Assets/" + seqUUID + "/AssetData/" + fullLang, dirs_exist_ok=True)
			#motion files
			shutil.rmtree(os.path.join(pathlib.Path.cwd()) + "/__OverriddenContent__/VideoAppsClipsAssets/Assets/" + motiUUID + "/AssetData/" + fullLang)
			shutil.copytree(os.path.join(pathlib.Path.cwd()) + '/' + i + '/PosterFrame/', os.path.join(pathlib.Path.cwd()) + "/__OverriddenContent__/VideoAppsClipsAssets/Assets/" + motiUUID + "/AssetData/" + fullLang + "/Media", dirs_exist_ok=True)
			shutil.copy2(os.path.join(pathlib.Path.cwd()) + '/' + i + '/' + i + ".moti", os.path.join(pathlib.Path.cwd()) + "/__OverriddenContent__/VideoAppsClipsAssets/Assets/" + motiUUID + "/AssetData/" + fullLang + "/" + ISOLang + "_LoopStart.moti")
			

		for i in incomingAssets:
			#remove errant poster frame from media folder
			os.remove(os.path.join(pathlib.Path.cwd()) + '/' + i + '/Media/' + i + '_Poster_Frame.png')
			if assetNumber == 1:
		...
		subfolders = [ f.path for f in os.scandir(os.path.join(pathlib.Path.cwd())) if f.is_dir() ]
	...
	def video_to_frames(self):
		...
		for directories in subfolders:
			shutil.rmtree(directories)
		assetList = []
		# TODO: pathlib.Path.cwd() to `input_path`
		for file in os.listdir(pathlib.Path.cwd()):
			if file.endswith(".mov"):
				print(os.path.join(pathlib.Path.cwd(), file))
				os.makedirs(os.path.splitext(file)[0] + '/Media/')
				os.makedirs(os.path.splitext(file)[0] + '/PosterFrame/')
				...
				.filter('scale', 720, -1)
				.output(os.path.splitext(file)[0] + '/Media/' + "%s_%%05d.png"%(input_file_name[:-4]), start_number=0, pix_fmt='rgba')
				.run())
				...
				numberOfFiles = next(os.walk(os.path.splitext(file)[0] + '/Media/'))[2]
				...
				shutil.copy2(os.path.splitext(file)[0] + '/Media/' + posterFrame, os.path.splitext(file)[0] + '/PosterFrame/'+ os.path.splitext(file)[0] + "_Poster_Frame.png") # complete target filename given
		return(assetList)
	...
	def main(self):
		...
		# return
		for top_dir_name in os.listdir("./"):
			self.target_dir = "./" + top_dir_name + "/" 
			...
			for file_name in glob.glob(self.target_dir + self.provided_poster_frame_dir + "/*.png"):
				print("I found a provided poster frame: " + file_name)
				provided_poster_frame_image = cv2.imread(file_name, cv2.IMREAD_COLOR)

			file_count = 0
			for file_name in glob.glob(self.target_dir + self.media_dir + "/*.png"):
			...
			# 
			for root, dirs, files in os.walk(self.target_dir + self.media_dir):
				...
				for f in files:
					if len(provided_poster_frame_image)>0:
						...
						tmp_frame = cv2.imread(self.target_dir + self.media_dir + "/" + f, cv2.IMREAD_COLOR)
						...
						if np.array_equiv(tmp_frame, provided_poster_frame_image):
							...
							path_to_frame = self.target_dir + self.media_dir + poster_frame_name
							path_to_frame = path_to_frame.replace(" ", "\ ")
							arg = "cp " + self.target_dir + self.media_dir + f + " " + path_to_frame
							os.system(arg)
				...
				if poster_frame_name == "":
					...
					self.get_hit_area_points(self.target_dir + "Media/" + poster_frame_name)
	...
	def fix_sequence_zeros_and_compare_first_and_last_frames(self):
		...
		for top_dir_name in os.listdir("./"):
			if os.path.isdir("./" + top_dir_name):
				self.target_dir = "./" + top_dir_name + "/" 
				# get number of PNGs in the current media folder
				pngCounter = len(glob.glob1(self.target_dir + self.media_dir, "*.png"))
				i=0
				while i < pngCounter:
					for file_name in sorted(glob.glob(self.target_dir + self.media_dir + "/*.png")):
		...
		for top_dir_name in os.listdir("./"):
			...
			if os.path.isdir("./" + top_dir_name):
				...
				if " " in top_dir_name or "_" in top_dir_name:
					...
					arg = "mv " + "./" + top_dir_name + " ./" + new_top_dir_name
					os.system(arg)
	...
	def change_subdir_to_media_dir_name(self):
		...
		for top_dir_name in os.listdir("./"):
			if os.path.isdir("./" + top_dir_name):
				for sub_dir_name in os.listdir("./" + top_dir_name + "/"):
					if "PNG Sequence" in sub_dir_name or "PNG" in sub_dir_name:
						arg = "mv " + "./" + top_dir_name + "/" + sub_dir_name.replace(" ", "\ ") + " ./" + top_dir_name + "/Media"
						os.system(arg)
